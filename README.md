...
**Учебная конфигурация 1С:Предприятие 8.3 для performance-агентства (iGaming)**

![Платформа](https://img.shields.io/badge/1С%3AПредприятие-8.3-yellow)
![Язык](https://img.shields.io/badge/синтаксис-русский-blue)
![СКД](https://img.shields.io/badge/отчёты-СКД-green)
![Статус](https://img.shields.io/badge/статус-учебный%20проект-lightgrey)
...
- **Заполнение тестовыми данными** — обработка перед созданием документа ищет существующий по ключевым полям (клиент, период), поэтому при повторном запуске дублей не появляется.
...
- Разработка шла итерациями: после каждой — коммит, пакетная загрузка
  конфигурации в базу (LoadConfigFromFiles) и проверка log.txt на ошибки.
...
| ![Цепочка документов](docs/screenshots/chain.png) | Цепочка документов: оплата привязана к акту (АктКлиенту → ОплатаОтКлиента) |
| ![Отказ контроля капы](docs/screenshots/kapa.png) | Отказ проведения при превышении капы лидов по офферу |

---

<div align="center">

*Учебный проект. Предметная область смоделирована самостоятельно, все данные синтетические.*
